...
namespace TaoxanhProxy
{
    public class ProxyTestController : IDisposable
    {
        ...
        {
            WriteToConsole(e.IsValid ? "valid cer" : "invalid cer", ConsoleColor.Red);
        ...
        Task ExplicitEndPoint_BeforeTunnelConnectRequest(object sender, TunnelConnectSessionEventArgs e)
        {
            var hostName = e.HttpClient.Request.RequestUri.Host;
            e.DecryptSsl = false;
            return Task.CompletedTask;
